...
CLANG_VER="clang-17.0.6"
LLVM_VER="llvm-17.0.6"
...
getllvm() {
  if [ ! -d dependencies/$LLVM_VER.src ]; then
    while true; do
      ...
      [Yy]*)
        cd dependencies || exit
        wget https://github.com/llvm/llvm-project/releases/download/llvmorg-17.0.6/$LLVM_VER.src.tar.xz
...
getclang() {
  if [ ! -d dependencies/$CLANG_VER.src ]; then
    while true; do
      ...
      [Yy]*)
        cd dependencies || exit
        wget https://github.com/llvm/llvm-project/releases/download/llvmorg-17.0.6/$CLANG_VER.src.tar.xz
